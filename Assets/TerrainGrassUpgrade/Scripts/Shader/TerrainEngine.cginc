...
//Obstacle Bending
uniform float _BendIntensity = 1.5;
uniform float4 _Obstacles[100];
uniform float _ObstacleRadius[100];
uniform float _ObstacleMax;

//Grass Sway 
uniform float _ShakeDisplacement = 1;
uniform float _ShakeTime = 1;
uniform float _ShakeWindspeed = 1;
uniform float _ShakeBending = 1;
...
appdata_full GrassBend(inout appdata_full v)
{
	float3 worldPos = mul(unity_ObjectToWorld, v.vertex);

	for (int i = 0; i < _ObstacleMax; i++) {

		float _Radius = _ObstacleRadius[i];
		float3 dist = distance(float3(worldPos.x, 0, worldPos.z), float3(_Obstacles[i].x, 0, _Obstacles[i].z));
		float3 circle = 1 - saturate(dist / _Radius);

		float3 sphereDisp = worldPos - float3(_Obstacles[i].x, 0, _Obstacles[i].z);
		sphereDisp *= circle;
		v.vertex.xz += sphereDisp.xz * _BendIntensity * (v.texcoord.y);
	}

	return v;
}

appdata_full GrassSway(inout appdata_full v)
{
...
void WavingGrassVert(inout appdata_full v)
{
	...
	//Realistic Grass
	v.vertex = GrassSway(v).vertex;

	//Obstacle avoidance
	v.vertex = GrassBend(v).vertex;
}
